...
The sitemap.xml only shows one url. What should I do?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
We use an index file of all available sitemaps. More information you'll get by
`Google <https://support.google.com/webmasters/answer/75712>`_. All sub sitemaps are also
generated by cs_seo, except the external ones. You can configure more sitemaps for other languages or
extensions via TypoScript :ref:`extend-sitemap`.